...
static gint32 _jm_kwp2000_set_options(JMKWP2000 *self, JMKWPMode msg_mode, JMKWPMode link_mode, gint32 baud) {
...
static gint32 _jm_kwp2000_set_address(JMKWP2000 *self, guint32 target_address, guint32 source_address) {
    g_return_val_if_fail(self != NULL, JM_ERROR_GENERIC);
...
static size_t _jm_kwp2000_pack(JMKWP2000 *self, const guint8 *src, size_t count, guint8 *tar) {
    size_t pos = 0;
    guint8 checksum = 0;
    size_t i = 0;
    ...
    if (self->mode == JM_KWP00) {
        tar[pos++] = 0x00;
        tar[pos++] = JM_LOW_BYTE(count);
        memcpy(tar + pos, src, count);
        pos += count;
    }
    if (self->mode == JM_KWPXX) {
        tar[pos++] = JM_LOW_BYTE(count);
        memcpy(tar + pos, src, count);
        pos += count;
    }
...
static size_t _jm_kwp2000_unpack(JMKWP2000 *self, const guint8 *src, size_t count, guint8 *tar) {
    size_t length = 0;
    ...
    if (src[0] > 0x80) {
        ...
        if (length != (count - JM_KWP8X_HEADER_LENGTH - JM_KWP_CHECKSUM_LENGTH)) {
            length = src[0] - 0xC0; /* For KWPCX */
            if (length != (count - JM_KWPCX_HEADER_LENGTH - JM_KWP_CHECKSUM_LENGTH)) {
                return 0;
            }
        }
        ...
        if (length != (count - JM_KWP80_HEADER_LENGTH - JM_KWP_CHECKSUM_LENGTH)) {
            return 0;
        }
        memcpy(tar, src + JM_KWP80_HEADER_LENGTH, length);
    } else if (src[0] == 0x00) {
        length = src[1];
        if (length != (count - JM_KWP00_HEADER_LENGTH - JM_KWP_CHECKSUM_LENGTH)) {
            return 0;
        }
...
gint32 jm_kwp2000_set_address(JMKWP2000 *self, guint8 target, guint8 source) {
    g_return_val_if_fail(self != NULL, JM_ERROR_GENERIC);
