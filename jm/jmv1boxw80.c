...
static gboolean _jm_v1_box_w80_check_result(JMV1Box *self, gint64 microseconds) {
    guint8 rb = 0;
    g_return_val_if_fail(self != NULL, FALSE);
...
static gboolean _jm_v1_box_w80_send_cmd(JMV1Box *self, guint8 cmd, size_t count, const guint8 *data) {
    JMV1BoxW80 *w80 = NULL;
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (jm_commbox_port_write(command->data, command->len) != command->len) {
            continue;
        }
...
static size_t _jm_v1_box_w80_read_data(JMV1Box *self, guint8 *buff, size_t count, gint64 microseconds) {
    size_t avail = 0;
    size_t len = 0;
...
static size_t _jm_v1_box_w80_read_bytes(JMV1Box *self, guint8 *buff, size_t count) {
    return _jm_v1_box_w80_read_data(self, buff, count, JM_TIMER_TO_MS(500));
}

static guint8 _jm_v1_box_w80_get_cmd_data(JMV1Box *self, guint8 *receive_buffer, size_t count) {
    guint8 len = 0;
    guint8 cs = 0;
    g_return_val_if_fail(self != NULL, 0);
...
static gboolean _jm_v1_box_w80_do_cmd(JMV1Box *self, guint8 cmd, size_t count, const guint8 *buff) {
    JMV1BoxW80 *w80 = NULL;
    gboolean ret = FALSE;
    g_return_val_if_fail(self != NULL, FALSE);
    w80 = (JMV1BoxW80*)self->user_data;
    ...
    if (self->shared->is_do_now) {
        ...
        if (cmd == JM_V1_BOX_W80_WR_DATA) {
            ...
            ret = _jm_v1_box_w80_send_cmd(self, JM_V1_BOX_W80_WR_DATA, temp->len, temp->data);
            ...
            return _jm_v1_box_w80_send_cmd(self, JM_V1_BOX_W80_DO_BAT_C, 0, NULL);
        } else {
...
static gboolean _jm_v1_box_w80_do_set(JMV1Box *self, guint8 cmd, size_t count, const guint8 *buff) {
...
static guint8 _jm_v1_box_w80_get_buff_data(JMV1Box *self, guint8 addr, guint8 *buff, size_t count) {
    guint8 temp[2];
    g_return_val_if_fail(self != NULL, 0);
    temp[0] = addr;
    temp[1] = count;
...
static gboolean _jm_v1_box_w80_set_line_level(JMV1Box *self, guint8 value_low, guint8 value_high) {
    JMV1BoxW80 *w80 = NULL;
    g_return_val_if_fail(self != NULL, FALSE);
    ...
    return _jm_v1_box_w80_do_set(self, JM_V1_BOX_W80_SET_PORT1, 1, w80->ports + 1);
}

static gboolean _jm_v1_box_w80_set_comm_ctrl(JMV1Box *self, guint8 value_open, guint8 value_close) {
    JMV1BoxW80 *w80 = NULL;
    g_return_val_if_fail(self != NULL, FALSE);
    ...
    return _jm_v1_box_w80_do_set(self, JM_V1_BOX_W80_SET_PORT2, 1, w80->ports + 2);
}

static gboolean _jm_v1_box_w80_set_comm_line(JMV1Box *self, guint8 send_line, guint8 recv_line) {
    JMV1BoxW80 *w80 = NULL;
    g_return_val_if_fail(self != NULL, FALSE);
    ...
    return _jm_v1_box_w80_do_set(self, JM_V1_BOX_W80_SET_PORT0, 1, w80->ports);
}
...
static gboolean _jm_v1_box_w80_keep_link(JMV1Box *self, gboolean is_run_link) {
    if (is_run_link) {
        return _jm_v1_box_w80_do_set(self, JM_V1_BOX_W80_RUN_LINK, 0, NULL);
    } else {
        return _jm_v1_box_w80_do_set(self, JM_V1_BOX_W80_STOP_LINK, 0, NULL);
    }
}

static gboolean _jm_v1_box_w80_set_comm_link(JMV1Box *self, guint8 ctrl_word1, guint8 ctrl_word2, guint8 ctrl_word3) {
    guint8 ctrl_word[3];
    ...
    if (mode_control == JM_V1_BOX_W80_SET_VPW || mode_control == JM_V1_BOX_W80_SET_PWM) {
        return _jm_v1_box_w80_do_set(self, JM_V1_BOX_W80_SET_CTRL, 1, ctrl_word);
    }
    ...
    return _jm_v1_box_w80_do_set(self, JM_V1_BOX_W80_SET_CTRL, length, ctrl_word);
}
...
static gboolean _jm_v1_box_w80_set_comm_baud(JMV1Box *self, gdouble baud) {
    ...
    if (baud_time[0] == 0) {
        return _jm_v1_box_w80_do_set(self, JM_V1_BOX_W80_SET_BAUD, 1, baud_time + 1);
    }
    return _jm_v1_box_w80_do_set(self, JM_V1_BOX_W80_SET_BAUD, 2, baud_time);
}

static void _jm_v1_box_w80_get_link_time(JMV1Box *self, guint8 type, guint32 time) {
...
static gboolean _jm_v1_box_w80_set_comm_time(JMV1Box *self, guint8 type, guint32 time) {
    guint8 time_buff[2];
    JMV1BoxW80 *w80 = NULL;
    ...
    if (type == JM_V1_BOX_W80_SETVPWSTART || type == JM_V1_BOX_W80_SETVPWRECS) {
        ...
        time = (guint32)((time / w80->time_base_db) / (w80->box_time_unit / 1000000.0));
    }
...
static gboolean _jm_v1_box_w80_commbox_delay(JMV1Box *self, guint32 time) {
    ...
    if (time > 65535) {
        ...
        if (time > 65535) {
            time = (guint32)((time * w80->time_base_db) / w80->time_extern_db);
            if (time > 65535) {
                return FALSE;
            }
...
static gboolean _jm_v1_box_w80_send_out_data(JMV1Box *self, const guint8 *buffer, size_t count) {
    return _jm_v1_box_w80_do_set(self, JM_V1_BOX_W80_SEND_DATA, count, buffer);
}
...
static gboolean _jm_v1_box_w80_stop_now(JMV1Box *self, gboolean is_stop_execute) {
    guint8 cmd;
    size_t i;
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (_jm_v1_box_w80_check_send(self)) {
            if (is_stop_execute && !_jm_v1_box_w80_check_result(self, JM_TIMER_TO_MS(200)))
                continue;
            return TRUE;
        }
    }
...
static gboolean _jm_v1_box_w80_set_rf(JMV1Box *self, guint8 cmd, guint8 data) {
    return FALSE;
}
...
static guint8 _jm_v1_box_w80_get_abs_add(JMV1Box *self, guint8 buff_id, guint8 add) {
    return 0;
}

static gboolean _jm_v1_box_w80_update_buff(JMV1Box *self, guint8 type, guint8 *buffer) {
    int len = 0;
    guint8 buf[3];
    buf[0] = buffer[0];
    buf[1] = buffer[1];

    if ((type == JM_V1_BOX_W80_INC_BYTE) || (type == JM_V1_BOX_W80_DEC_BYTE) || (type == JM_V1_BOX_W80_INVERTBYTE)) {
        len = 1;
    } else if ((type == JM_V1_BOX_W80_UPDATE_BYTE) || (type == JM_V1_BOX_W80_ADD_BYTE) || (type == JM_V1_BOX_W80_SUB_BYTE)) {
        len = 2;
    } else {
        len = 3;
    }
...
static gboolean _jm_v1_box_w80_copy_buff(JMV1Box *self, guint8 dest, guint8 src, guint8 len) {
    guint8 buf[3];
...
static gboolean _jm_v1_box_w80_end_batch(JMV1Box *self) {
    ...
    if (w80->is_link) {
        ...
        while (w80->buf[i] != 0) {
            ...
            if (w80->buf[i] == JM_V1_BOX_W80_SEND_DATA) {
                i += 1 + w80->buf[i + 1] + 1 + 1;
            } else if(w80->buf[i] >= JM_V1_BOX_W80_REC_LEN_1 && w80->buf[i] <= JM_V1_BOX_W80_REC_LEN_15) {
                i++; // 特殊 
            } else {
    ...
    return _jm_v1_box_w80_do_cmd(self, JM_V1_BOX_W80_WR_DATA, w80->pos, w80->buf);
}
...
static gboolean _jm_v1_box_w80_run_batch(JMV1Box *self, guint8 *buff_id, size_t count, gboolean is_execute_many) {
    guint8 cmd;
...
static gboolean _jm_v1_box_w80_check_serial_port_change_config(JMV1Box *self) {
    return TRUE;
}
...
JMV1Box* jm_v1_box_w80_new(JMV1Shared *shared) {
    ...
    obj->check_serial_port_change_config = _jm_v1_box_w80_check_serial_port_change_config;
    ...
    obj->serial_port_change_config = _jm_v1_box_w80_serial_port_change_config;
    obj->serial_port_databits = _jm_v1_box_w80_serial_port_databits;
    obj->serial_port_flow_control = _jm_v1_box_w80_serial_port_flow_control;
