    ...
    gpointer       user_data)
{
#ifdef _MSC_VER
    {
        gunichar2* uni_msg = g_utf8_to_utf16(message, -1, NULL, NULL, NULL);
    ...
    {
        gsize bytes_written;
        g_io_channel_write_chars(log_file, message, g_utf8_strlen(message, -1), &bytes_written, NULL);
...
{
    g_log_set_handler(JM_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, _jm_log_debug_handler, NULL);
#ifdef JM_SDK
