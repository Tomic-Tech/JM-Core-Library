...
gint32 jm_serial_port_set_baudrate(JMSerialPort *self, gint32 baudrate) {
	...
	if (!SetCommConfig(self->handle, &(self->comm_config), sizeof(COMMCONFIG))) {
		g_static_rec_mutex_unlock(self->mutex);
		return GetLastError();
	}
...
gint32 jm_serial_port_set_parity(JMSerialPort *self, gint32 parity) {
    ...
    if (!(parity == JM_SP_PAR_NONE || parity == JM_SP_PAR_SPACE || parity == JM_SP_PAR_EVEN || parity == JM_SP_PAR_ODD || parity == JM_SP_PAR_MARK)) {
        g_static_rec_mutex_unlock(self->mutex);
        return JM_ERROR_GENERIC;
    }
...
gint32 jm_serial_port_set_flow_control(JMSerialPort *self, gint32 flow_control) {
    ...
    if (!(flow_control == JM_SP_FC_NONE || flow_control == JM_SP_FC_HARDWARE || flow_control == JM_SP_FC_SOFTWARE)) {
        g_static_rec_mutex_unlock(self->mutex);
        return JM_ERROR_GENERIC;
    }
...
gint32 jm_serial_port_open(JMSerialPort *self) {
    ...
    /* Open the port */
    self->handle = CreateFileA(self->port_name, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, dw_flags_and_attribute, NULL);
    ...
    if (!GetCommConfig(self->handle, &(self->comm_config), &conf_size) || !GetCommState(self->handle, &(self->comm_config.dcb))) {
        g_static_rec_mutex_unlock(self->mutex);
        return GetLastError();
    }
...
size_t jm_serial_port_read(JMSerialPort *self, guint8 *data, size_t count) {
    ...
    if (!ReadFile(self->handle, (void*)data, (DWORD)count, &ret_val, &(self->read_overlap))) {
        if (GetLastError() != ERROR_IO_PENDING) {
            g_static_rec_mutex_unlock(self->mutex);
            return 0;
        }
        GetOverlappedResult(self->handle, &(self->read_overlap), &ret_val, TRUE);
    }
...
size_t jm_serial_port_write(JMSerialPort *self, const guint8 *data, size_t count) {
    ...
    if (!WriteFile(self->handle, (const void*)data, (DWORD)count, &ret_val, &(self->write_overlap))) {
        if (GetLastError() != ERROR_IO_PENDING) {
            g_static_rec_mutex_unlock(self->mutex);
            return 0;
        }
        WaitForSingleObject(self->write_overlap.hEvent, INFINITE);
        GetOverlappedResult(self->handle, &(self->write_overlap), &ret_val, TRUE);
    }
