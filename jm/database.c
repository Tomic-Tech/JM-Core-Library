...
Database * database_new(const gchar *filePath)
{
    ...
    if (sqlite3_open_v2(filePath, &obj->handle, SQLITE_OPEN_READONLY, NULL) != SQLITE_OK) {
        obj->handle = NULL;
        g_free(obj);
        return NULL;
    }
...
gboolean database_get_text(Database *obj, const gchar *name, gunichar2 *text)
{
    ...
    if (obj->getTextStmt == NULL) {
        ...
        ret = sqlite3_prepare_v2(obj->handle, temp->str, temp->len, &obj->getTextStmt, NULL);
    ...
    if ((sqlite3_reset(obj->getTextStmt) == SQLITE_OK) &&
        (sqlite3_bind_text(obj->getTextStmt, 1, name, strlen(name), SQLITE_STATIC) == SQLITE_OK) &&
        (sqlite3_step(obj->getTextStmt) == SQLITE_ROW)) {
        const gunichar2 *temp = (const gunichar2 *) sqlite3_column_text16(obj->getTextStmt, 0);
        if (temp != NULL)
			memcpy(text, temp, sqlite3_column_bytes16(obj->getTextStmt, 0) + sizeof(gunichar2));
    }

    return TRUE;
}

gboolean database_get_trouble_code(Database *obj, const gchar *code, gunichar2 *text)
{
	long items_read = 0;
	long items_written = 0;
	gunichar2 *temp = NULL;
    ...
    if (obj->getTroubleCodeStmt == NULL) {
        ...
        ret = sqlite3_prepare_v2(obj->handle, temp->str, temp->len, &obj->getTroubleCodeStmt, NULL);
        g_string_free(temp, TRUE);
        if (ret != SQLITE_OK) {
    ...
    if ((sqlite3_reset(obj->getTroubleCodeStmt) == SQLITE_OK) &&
        (sqlite3_bind_text(obj->getTroubleCodeStmt, 1, code, strlen(code), SQLITE_STATIC) == SQLITE_OK) &&
        (sqlite3_bind_text(obj->getTroubleCodeStmt, 2, obj->tcCatalog->str, obj->tcCatalog->len, SQLITE_STATIC) == SQLITE_OK)) {
        int rc = sqlite3_step(obj->getTroubleCodeStmt);
        if (rc == SQLITE_ROW) {
            const gunichar2* temp = (const gunichar2*) sqlite3_column_text16(obj->getTroubleCodeStmt, 0);
            if (temp) {
				memcpy(text, temp, sqlite3_column_bytes16(obj->getTroubleCodeStmt, 0) + sizeof(gunichar2));
            } else {
...
gboolean database_get_command(Database *obj, const gchar *name, guint8 *buffer, size_t *count)
{
    g_return_val_if_fail(obj != NULL, FALSE);

    if (obj->getCommandStmt == NULL) {
        static const char *temp = "SELECT Command FROM [Command] WHERE Name=:name AND Catalog=:catalog";
        int ret;
        ret = sqlite3_prepare_v2(obj->handle, temp, strlen(temp), &obj->getCommandStmt, NULL);
        if (ret != SQLITE_OK) {
    ...
    if ((sqlite3_reset(obj->getCommandStmt) == SQLITE_OK) &&
        (sqlite3_bind_text(obj->getCommandStmt, 1, name, strlen(name), SQLITE_STATIC) == SQLITE_OK) &&
        (sqlite3_bind_text(obj->getCommandStmt, 2, obj->cmdCatalog->str, obj->cmdCatalog->len, SQLITE_STATIC) == SQLITE_OK) &&
        (sqlite3_step(obj->getCommandStmt) == SQLITE_ROW)) {
        const guint8 *bytes = (const guint8*) sqlite3_column_blob(obj->getCommandStmt, 0);
...
gboolean database_get_command_by_id(Database *obj, gint32 id, guint8 *buffer, size_t *count)
{
    g_return_val_if_fail(obj != NULL, FALSE);
    
    if (obj->getCommandByIDStmt == NULL) {
        static const gchar *text = "SELECT Command FROM [Command] WHERE ID=:id";
        int ret = sqlite3_prepare_v2(obj->handle, text, strlen(text), &obj->getCommandByIDStmt, NULL);
        if (ret != SQLITE_OK) {
    ...
    if ((sqlite3_reset(obj->getCommandByIDStmt) == SQLITE_OK) &&
        ...
        const guint8 *bytes = (const guint8*) sqlite3_column_blob(obj->getCommandByIDStmt, 0);
