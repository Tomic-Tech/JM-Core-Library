...
bool v1_w80_box::send_cmd(uint8 cmd, int32 offset, int32 count, const uint8 *data) {
    uint8 cs = cmd;
    cmd += _run_flag;
    byte_array command;
...
int32 v1_w80_box::read_data(uint8 *buff, int32 offset, int32 count, int64 microseconds) {
...
int32 v1_w80_box::read_bytes(uint8 *buff, int32 offset, int32 count) {
    return read_data(buff, offset, count, timer::to_ms(500));
}

uint8 v1_w80_box::get_cmd_data(uint8 *receive_buffer, int32 count) {
    uint8 len = 0;
    uint8 cs = 0;
...
bool v1_w80_box::do_cmd(uint8 cmd, int32 offset, int32 count, const uint8 *buff) {
    _start_pos = 0;
    ...
    if (get_shared()->is_do_now) {
        ...
        if (cmd == WR_DATA) {
            ...
            return send_cmd(WR_DATA, 0, temp.size(), temp.data());
            ...
            temp.push_back(SEND_DATA); //命令
            temp.push_back(count - 1); //命令长度-1
            temp.push_back(buff + offset, count);
            if (!send_cmd(WR_DATA, 0, temp.size(), temp.data()))
                return false;
            return send_cmd(DO_BAT_C, 0, 0, NULL);
        } else {
...
bool v1_w80_box::do_set(uint8 cmd, int32 offset, int32 count, const uint8 *buff) {
...
uint8 v1_w80_box::get_buff_data(uint8 addr, uint8 *buff, int32 count) {
    uint8 temp[2];
    temp[0] = addr;
    temp[1] = count;
    if (!do_cmd(GET_BUF, 0, 2, temp)) {
        return 0;
    }
...
bool v1_w80_box::set_comm_line(uint8 send_line, uint8 recv_line) {
    ...
    return do_set(SET_PORT0, 0, 1, _ports);
}

bool v1_w80_box::turn_over_one_by_one() {
    return do_set(SET_ONEBYONE, 0, 0, NULL);
}

bool v1_w80_box::keep_link(bool is_run_link) {
    if (is_run_link) {
        return do_set(RUN_LINK, 0, 0, NULL);
    } else {
        return do_set(STOP_LINK, 0, 0, NULL);
    }
}
...
bool v1_w80_box::set_comm_link(uint8 ctrl_word1, uint8 ctrl_word2, uint8 ctrl_word3) {
    ...
    if (mode_control == SET_VPW || mode_control == SET_PWM) {
        return do_set(SET_CTRL, 0, 1, ctrl_word);
    }
    ...
    return do_set(SET_CTRL, 0, length, ctrl_word);
}
...
bool v1_w80_box::set_comm_baud(double baud) {
    ...
    if (baud_time[0] == 0) {
        return do_set(SET_BAUD, 1, 1, baud_time);
    }
    return do_set(SET_BAUD, 0, 2, baud_time);
}
...
bool v1_w80_box::send_out_data(const uint8 *buffer, int32 offset, int32 count) {
    return do_set(SEND_DATA, offset, count, buffer);
}
...
bool v1_w80_box::update_buff(uint8 type, uint8 *buffer) {
    ...
    buf[0] = buffer[0];
    buf[1] = buffer[1];
    if ((type == INC_BYTE) || (type == DEC_BYTE) || (type == INVERTBYTE)) {
        len = 1;
    } else if ((type == UPDATE_BYTE) || (type == ADD_BYTE) || (type == SUB_BYTE)) {
        len = 2;
    } else {
        len = 3;
    }
...
bool v1_w80_box::copy_buff(uint8 dest, uint8 src, uint8 len) {
    ...
    buf[1] = src;
    buf[2] = len;
    return do_set(COPY_BYTE, 0, 3, buf);
}

bool v1_w80_box::new_batch(uint8 buff_id) {
    _pos = 0;
    _is_link = (buff_id == v1_shared::LINKBLOCK ? true : false);
...
bool v1_w80_box::end_batch() {
    ...
    if (_is_link) { //修改update_buff使用到的地址
        while (_buf[i] != 0) {
            ...
            if (mode == COPY_BYTE) {
                _buf[i + 3] = _buf[i + 3] + MAXBUFF_LEN - _pos;
            } else if (mode == SUB_BYTE) {
                _buf[i + 2] = _buf[i + 2] + MAXBUFF_LEN - _pos;
            } else {
                _buf[i + 1] = _buf[i + 1] + MAXBUFF_LEN - _pos;

            }
            if (_buf[i] == SEND_DATA) {
                i += 1 + _buf[i + 1] + 1 + 1;
            } else if (_buf[i] >= v1_shared::REC_LEN_1 && _buf[i] <= v1_shared::REC_LEN_15) {
                i++; //特殊
            } else {
                i = i + ((_buf[i] & 0x03) + 1);
            }
    ...
    return do_cmd(WR_DATA, 0, _pos, _buf);
}
...
bool v1_w80_box::run_batch(uint8 *buff_id, int32 count, bool is_execute_many) {
    int cmd;
    if (buff_id[0] == v1_shared::LINKBLOCK) {
        if (is_execute_many) {
            cmd = DO_BAT_LN;
        } else {
            cmd = DO_BAT_L;
