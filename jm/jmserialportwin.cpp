...
namespace JM
{
    ...
    void SerialPort::platformInit()
    {
        ...
        ZeroMemory(&_writeOverlap, sizeof(OVERLAPPED));
        _readOverlap.hEvent = CreateEvent(NULL, TRUE, TRUE, NULL);
        _writeOverlap.hEvent = CreateEvent(NULL, TRUE, TRUE, NULL);
        setPortName("\\\\.\\COM1");
    }

    void SerialPort::platformDestroy()
    {
        CloseHandle(_readOverlap.hEvent);
        CloseHandle(_writeOverlap.hEvent);
    }
    ...
    gint32 SerialPort::setBaudrate(gint32 baudrate)
    {
        ...
        if (!SetCommConfig(_handle, &(_commConfig), 
            sizeof(COMMCONFIG)))
        {
            g_static_rec_mutex_unlock(_mutex);
            return GetLastError();
    ...
    gint32 SerialPort::open()
    {
        ...
        if (!GetCommConfig(_handle, &(_commConfig), &conf_size) || 
    ...
    size_t SerialPort::read(guint8 *data, size_t count)
    {
        ...
        if (!ReadFile(_handle, (void*)data, (DWORD)count, &ret_val, 
            &(_readOverlap)))
        {
            ...
            GetOverlappedResult(_handle, &(_readOverlap), &ret_val, 
                TRUE);
        }
    ...
    size_t SerialPort::write(const guint8 *data, size_t count)
    {
        ...
        if (!WriteFile(_handle, (const void*)data, (DWORD)count, &ret_val, 
            &(_writeOverlap)))
        {
            if (GetLastError() != ERROR_IO_PENDING)
            {
            ...
            WaitForSingleObject(_writeOverlap.hEvent, INFINITE);
            GetOverlappedResult(_handle, &(_writeOverlap), &ret_val, 
                TRUE);
        }
