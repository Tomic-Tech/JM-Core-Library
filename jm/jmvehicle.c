...
{
    g_thread_join(_jm_vehicle_commbox_close_thread);
}
...
{
    return _jm_vehicle_path->str;
}

static gboolean _jm_open_db_first(const gchar *path, const gchar *db_name)
{
    GString *abs_db_path = g_string_new(_jm_vehicle_path->str);
    gchar *pw = NULL;
    gboolean ret = FALSE;
