    ...
    namespace V1
    {
        namespace C168
        {
            ...
            public:
                ...
                // CommBox 固定信息 宏定义表
                //////////////////////////////////////////////////////////////////////////
                static const guint8 NULLADD = 0xFF; // 表示此块无使用
                static const guint8 MAXIM_BLOCK = 0x40; // 命令缓从区的最大数
                static const guint8 SWAPBLOCK = (MAXIM_BLOCK + 1); // 数据交换区的块表识
                static const guint32 START_BAUD = 57600; // 上位机同下位机通信在复位或上电时波特率为57600
                static const guint8 CMD_DATALEN = 4; // 非发送命令最大长度
                // /////////////////////////////////////////////////////
                /*
                ...
                ***************************************************************************/
                // 1、写入命令缓冲区命令：
                static const guint8 WR_DATA = 0xD0; // 写缓冲区命令字,写入数据到命令缓冲区
                static const guint8 WR_LINK = 0xFF; // 若写入命令的地址为WR_LINK，写入数据到链路保持区链路保持区存放在命令缓冲区最后,存放次序:按地址从低到高
                static const guint8 SEND_LEN = 0xA8; // 一次发送数据的数据长度,0X70个数据
                static const guint8 SEND_DATA = 0x00;
                // 2、单字节命令：（大于写入命令缓冲区命令字 WR_DATA 0xD0，皆为单字节命令区）
                // 非缓冲区命令
                static const guint8 RESET = 0xF3; // 软件复位命令 清除所有缓冲区和寄存器内容。
                static const guint8 GETINFO = 0xF4;
                /*
                ...
                */
                // 中断命令定义
                static const guint8 STOP_REC = 0xFB; // 中断接受命令 强行退出当前接受命令，不返回错误。(中断命令)
                static const guint8 STOP_EXECUTE = 0xFC; // 中断批处理命令
                // 在当前执行时，通过该命令停止当前接受操作，返回错误。(中断命令)
                // 单字节缓冲区命令
                // static const guint8 GET_PORT1 0xD8 //等到通讯口的当前状态
                static const guint8 SET_ONEBYONE = 0xD9; // 将原有的接受一个发送一个的标志翻转
                ...
                */
                static const guint8 RUNLINK = 0xF1; // 启动链路保持 F1 启动链路保持，定时执行链路保持内容，在每次执行前回传链路保持开始信号，结束时回传链路保持结束信号。
                static const guint8 STOPLINK = 0xF2; // 中断链路保持 F2 结束链路保持执行。
                static const guint8 CLR_LINK = 0xDE; // 清除链路保持缓冲区
                static const guint8 DO_BAT_00 = 0xDF; // 批处理命令，执行一次命令缓冲区00地址的命令区
                // 3、多字节命令：（命令空间 0x30-= 0xCF）
                static const guint8 D0_BAT = 0x78; // 批处理命令，连续执行一次最多4块命令缓冲区的地址命令区；数据最多为4个命令区的首地址
                static const guint8 D0_BAT_FOR = 0x7C; // 批处理命令，连续执行无数次最多4块命令缓冲区的地址命令区；数据最多为4个命令区的首地址
                // 多字节命令
                static const guint8 SETTING = 0x80; // 下位机通讯链路状态字设定：设定3个通讯控制字，无用设定或没有设定都自动清零
                static const guint8 SETBAUD = 0x84; // 通讯波特率设定，只用2个数据位，单字节为低字节，双字节高字节在前，低字节在后。
                ...
                */
                static const guint8 ECHO = 0x9C; // 回传指定数据，按序回传数据
                static const guint8 SETPORT0 = 0xA0; // 只有一个字节的数据，设定端口0
                static const guint8 SETPORT1 = 0xA4; // 只有一个字节的数据，设定端口1
                static const guint8 SETPORT2 = 0xA8; // 只有一个字节的数据，设定端口2
                static const guint8 SETPORT3 = 0xAC; // 只有一个字节的数据，设定端口3
                // 已删除#define SETALLPORT 0x6F //只有四个字节的数据，设定端口0，1，2，3
                static const guint8 SET_UPBAUD = 0xB0; // 设置上位机的通讯波特率 ,仅有数据位1位,定义如下:其他非法

                /*
                * #define UP_9600BPS 0x00 #define UP_19200BPS 0x01 #define UP_38400BPS 0x02
                * #define UP_57600BPS 0x03 #define UP_115200BPS 0x04
                */
                static const guint8 DELAYSHORT = 0xB4; // 设定延时时间
                // (DB20)只用2个数据位，单字节为低字节，双字节高字节在前，低字节在后。
                static const guint8 DELAYTIME = 0xB8; // 设定延时时间
                // (DB20)只用2个数据位，单字节为低字节，双字节高字节在前，低字节在后。
                static const guint8 DELAYLONG = 0xBC; // 设定延时时间 (DB200)
                ...
                */
                // 取数据
                static const guint8 GET_DATA = 0xC8; // 88 地址 返回数据缓冲区指定的数据
                /***************************************************************************
                ...
                */
                ...
                // 时间计算设置Commbox宏定义区
                //////////////////////////////////////////////////////////////////////////
                static const guint8 REPLAYTIMES = 3; // 错误运行次数
                //////////////////////////////////////////////////////////////////////////
                // 设置Commbox宏定义区
                //////////////////////////////////////////////////////////////////////////
                static const guint32 TIMEVALUE = 1000000; // 万分之一秒微妙
                static const guint8 COMMBOXINFOLEN = 18; // 共有18个数据需从COMMBOX得到
                static const guint8 VERSIONLEN = 2;
                static const guint8 MINITIMELEN = 3;
                static const guint8 COMMBOXPORTNUM = 4;
                static const guint8 COMMBOXIDLEN = 10;
                static const guint8 LINKBLOCK = MAXIM_BLOCK; //链路保持的命令缓冲区
                // 批处理执行次数
                ...
                // RF多对一的设定接口
                static const guint8 SETDTR_L = 0x02;
                static const guint8 SETDTR_H = 0x03;
                static const guint8 MAX_RFADD = 0x2F; // 0x00-= 0x2F间的= 0x30个地址
                static const guint8 SETADD = 0x10; // 切换无线通讯设备到新地址
                static const guint8 CHANGEADD = 0x40; // 改变当前与之通讯的无线设备的地址
                static const guint8 SETRFBAUD = 0x04; // 改变无线串口通讯波特率
                static const guint8 RESET_RF = 0x00; // 复位无线通讯主设备，该命令需在9600波特率下实现
                //////////////////////////////////////////////////////////////////////////
                // 通讯口 PORT
                //////////////////////////////////////////////////////////////////////////
                ...
                static const guint8 ORIGINALBYTE = 0x00;
                //////////////////////////////////////////////////////////////////////////
                // 通讯设置参数时间
                //////////////////////////////////////////////////////////////////////////
                static const guint8 SETBYTETIME = 0x88; // 字节间时间设定 db20?，只用2个数据位，单字节为低字节，双字节高字节在前，低字节在后。
                static const guint8 SETVPWSTART = 0x08; // 设置vpw发送数据时需发送0的时间。
                static const guint8 SETWAITTIME = 0x8C; // 空闲等待时间设定db20?，只用2个数据位，单字节为低字节，双字节高字节在前，低字节在后。
                static const guint8 SETLINKTIME = 0x90; // 链路保持时建设定db20?，只用2个数据位，单字节为低字节，双字节高字节在前，低字节在后。
                static const guint8 SETRECBBOUT = 0x94; // 接受字节超时错误判断 db20（vpw为指令数），只用2个数据位，单字节为低字节，双字节高字节在前，低字节在后。
                static const guint8 SETRECFROUT = 0x98; // 接受一帧超时错误判断?db20?，只用2个数据位，单字节为低字节，双字节高字节在前，低字节在后。
                static const guint8 SETVPWRECS = 0x14;
                //////////////////////////////////////////////////////////////////////////
                // 上下位机通讯波特率
                //////////////////////////////////////////////////////////////////////////
                static const guint8 UP_9600BPS = 0x0;
                ...
                //////////////////////////////////////////////////////////////////////////
                // 数据拷贝
                static const guint8 COPY_DATA = 0xCA; // 8A 结果地址1 操作地址1 长度 COPY 操作地址1 TO 结果地址1
                // FOR 长度 字节
                // 修改数据
                static const guint8 UPDATE_1BYTE = 0xC1; // 81 结果地址 数据1 结果地址=数据1
                static const guint8 UPDATE_2BYTE = 0xC3; // 83 结果地址1 数据1 结果地址2 数据2 结果地址1=数据1
                // 结果地址2=数据2
                // 自增命令
                static const guint8 DEC_DATA = 0xC4; // 84 结果地址 结果地址=结果地址-1
                static const guint8 INC_DATA = 0xC0; // 80 结果地址 结果地址=结果地址+1
                static const guint8 INC_2DATA = 0xC5; // 85 结果地址1 结果地址2 结果地址1=结果地址1+1
                // 结果地址2=结果地址2+1
                // 加法命令
                static const guint8 ADD_1BYTE = 0xC2; // 82 结果地址 操作地址1 数据1 结果地址=操作地址1+数据1
                static const guint8 ADD_2BYTE = 0xC7; // 87 结果地址1 数据1 结果地址2 数据2 结果地址1=结果地址1+数据1
                // 结果地址2=结果地址2+数据2
                static const guint8 ADD_DATA = 0xC6; // 86 结果地址1 操作地址1 操作地址2 结果地址1=操作地址1+操作地址2
                // 减法命令
                static const guint8 SUB_DATA = 0xCE; // 8E 结果地址1 操作地址1 操作地址2 结果地址1=操作地址1-操作地址2
                static const guint8 SUB_BYTE = 0xCD; // 8D 结果地址1 数据1 结果地址1=数据1-结果地址1
                static const guint8 INVERT_DATA = 0xCC; // 8C 结果地址1 结果地址1=~结果地址
                //////////////////////////////////////////////////////////////////////////
                // 接受命令类型定义
                //////////////////////////////////////////////////////////////////////////
                static const guint8 GET_PORT1 = 0xD8; // 等到通讯口的当前状态
                static const guint8 SET55_BAUD = 0xDA; // 计算= 0x55的波特率
                static const guint8 REC_FR = 0xE0; // 接受一帧命令 E0 开始时回传开始接受信号，然后长期等待接受，接到数据实时回传，
                static const guint8 REC_LEN_1 = 0xE1; // 接受1个数据，返回
                static const guint8 REC_LEN_2 = 0xE2; // 接受2个数据，返回
                static const guint8 REC_LEN_3 = 0xE3; // 接受3个数据，返回
                static const guint8 REC_LEN_4 = 0xE4; // 接受4个数据，返回
                static const guint8 REC_LEN_5 = 0xE5; // 接受5个数据，返回
                static const guint8 REC_LEN_6 = 0xE6; // 接受6个数据，返回
                static const guint8 REC_LEN_7 = 0xE7; // 接受7个数据，返回
                static const guint8 REC_LEN_8 = 0xE8; // 接受8个数据，返回
                static const guint8 REC_LEN_9 = 0xE9; // 接受9个数据，返回
                static const guint8 REC_LEN_10 = 0xEA; // 接受10个数据，返回
                static const guint8 REC_LEN_11 = 0xEB; // 接受11个数据，返回
                static const guint8 REC_LEN_12 = 0xEC; // 接受12个数据，返回
                static const guint8 REC_LEN_13 = 0xED; // 接受13个数据，返回
                static const guint8 REC_LEN_14 = 0xEE; // 接受14个数据，返回
                static const guint8 REC_LEN_15 = 0xEF; // 接受15个数据，返回
                static const guint8 RECEIVE = 0xF0; // 连续接受 F0 开始时回传开始接受信号，然后长期等待接受，接到数据实时回传，
                //////////////////////////////////////////////////////////////////////////
                // ComBox记录信息和当前状态种类定义
                //////////////////////////////////////////////////////////////////////////
                static const guint8 GET_TIME = 0xF5; // 得到时间设定 DD返回字节时间、等待发送时间、链路保持时间、字节超时时间、接受超时时间
                static const guint8 GET_SET = 0xF6; // 得到链路设定 DE 返回链路控制字(3字节)、通讯波特率
                static const guint8 GET_PORT = 0xF7; // 得到端口设置 DF 返回端口p0，p1，p2，p3
                static const guint8 GET_LINKDATA = 0xF8; // 得到链路数据 FC 返回链路保持命令块中的所有内容 (中断命令)
                static const guint8 GET_BUFFDATA = 0xF9; // 得到缓冲器数据 FD 返回整个缓冲区数据 (中断命令)
                static const guint8 GET_COMMAND = 0xFA; // 得到命令数据 FE 返回上一执行命令。 (中断命令)
                //////////////////////////////////////////////////////////////////////////
                // 返回失败时，可根据Error_Record的值查找错误表定义
                //////////////////////////////////////////////////////////////////////////
                static const guint8 ILLIGICAL_LEN = 0xFF; //设置命令数据非法超长
                static const guint8 NOBUFF_TOSEND = 0xFE; //无交换缓冲区用于发送数据存放 
                static const guint8 SENDDATA_ERROR = 0xFD; //上位机发送数据异常
                static const guint8 CHECKSUM_ERROR = 0xFC; //接受命令回复校验和出错
                static const guint8 TIMEOUT_ERROR = 0xFB; //接受数据超时错误
                static const guint8 LOST_VERSIONDATA = 0xFA; //读到的Commbox数据长度不够.
                static const guint8 ILLIGICAL_CMD = 0xF9; //无此操作功能,没有定义.
                static const guint8 DISCONNECT_COMM = 0xF8; //没有连接上串口
                static const guint8 DISCONNECT_COMMBOX = 0xF7; //没有连接上COMMBOX设备
                static const guint8 NODEFINE_BUFF = 0xF6; //没有此命令块存在,未定义
                static const guint8 APPLICATION_NOW = 0xF5; //现有缓冲区申请,未取消,不能再此申请
                static const guint8 BUFFBUSING = 0xF4; //此缓冲区有数据未被撤销,不能使用,需删除此缓冲区,方可使用
                static const guint8 BUFFFLOW = 0xF3; //整个缓冲区无可使用的空间,不能申请,需删除缓冲区释放空间,方可使用
                static const guint8 NOAPPLICATBUFF = 0xF2; //未申请错误,需先申请,方可使用
                static const guint8 UNBUFF_CMD = 0xF1; //不是缓冲区命令,不能加载
                static const guint8 NOUSED_BUFF = 0xF0; //该缓冲区现没有使用,删除无效
                static const guint8 KEEPLINK_ERROR = 0xEF; //链路保持已断线
                static const guint8 UNDEFINE_CMD = 0xEE; //无效命令,未曾定义
                static const guint8 UNSET_EXRSBIT = 0xED; //没有设定扩展RS232的接受数据位个数
                static const guint8 COMMBAUD_OUT = 0xEC; //按照定义和倍增标志计算通讯波特率超出范围
                static const guint8 COMMTIME_OUT = 0xEB; //按照定义和倍增标志计算通讯时间超出范围
                static const guint8 OUTADDINBUFF = 0xEA; //缓冲区寻址越界
                static const guint8 COMMTIME_ZERO = 0xE9; //commbox时间基数为零
                static const guint8 SETTIME_ERROR = 0xE8; //延时时间为零
                static const guint8 NOADDDATA = 0xE7; //没有向申请的缓冲区填入命令,申请的缓冲区被撤销
                static const guint8 TESTNOLINK = 0xE6; //选择的线路没有连通
                static const guint8 PORTLEVELIDLE = 0xE5; //端口电平为常态
                static const guint8 COMMBOXID_ERR = 0xE4; //COMMBOX ID错误
                static const guint8 UP_TIMEOUT = 0xC0; //COMMBOX接受命令超时错误
                static const guint8 UP_DATAEER = 0xC1; //COMMBOX接受命令数据错误
                static const guint8 OVER_BUFF = 0xC2; //COMMBOX批处理缓冲区溢出,不判断链路保持数据是否会破坏缓冲区数据, 仅判断数据长度+数据地址>链路保持的开始位置成立溢出.*/
                static const guint8 ERROR_REC = 0xC3; //COMMBOX其他接受错误
                //COMMBOX执行操作错误
                static const guint8 SUCCESS = 0xAA; //COMMBOX执行成功
                static const guint8 SEND_OK = 0x55;
                static const guint8 RF_ERR = 0xC8;
                static const guint8 RUN_ERR = 0xC4; //COMMBOX运行启动检测错误
                //通讯通道定义
                ...
                static const guint8 SEND_CMD = 0x01;
            };
        }
    }
}
