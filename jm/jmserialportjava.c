...
JNIEXPORT jint Java_jm_io_SerialPort_nativeParity(JNIEnv *env, jclass cls, 
    jlong handle)
{
    JMSerialPort *port = (JMSerialPort*)handle;
    ...
    jlong handle, jbyteArray buff, jint offset, jint count)
{
    ...
    if (ret == count)
    {
        (*env)->SetByteArrayRegion(env, buff, offset, count, 
            (const jbyte*)temp);
    }
    g_free(temp);
    return ret;
    ...
    jlong handle, jbyteArray buff, jint offset, jint count)
{
    ...
    return jm_serial_port_write(port, (const guint8*)
        ((*env)->GetByteArrayElements(env, buff, NULL)) + offset, count);
}
    ...
    jclass cls)
{
    ...
    for (i = 0; i < jm_string_array_size(arr); i++)
    {
        (*env)->SetObjectArrayElement(env, ret, i, 
            (*env)->NewStringUTF(env, jm_string_array_get(arr, i)));
    }
    return ret;
}
