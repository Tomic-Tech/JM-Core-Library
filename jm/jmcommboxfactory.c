...
void jm_commbox_factory_init(void) {
	_jm_commbox_factory_obj_hash = g_hash_table_new_full(g_direct_hash, g_direct_equal, NULL, _jm_commbox_destroy);
}
...
JMCommbox* jm_commbox_factory_create_commbox() {
	...
	case JM_COMMBOX_V1:
		box = (JMCommbox*)g_hash_table_lookup(_jm_commbox_factory_obj_hash, GINT_TO_POINTER(_jm_commbox_factory_ver));
		if (box != NULL)
			break;

        box = jm_v1_commbox_new();
		g_hash_table_insert(_jm_commbox_factory_obj_hash, GINT_TO_POINTER(_jm_commbox_factory_ver), box);
		break;
	}
    return box;
}