#ifndef __JM_SYSTEM_IMPL_WIN_DISKINFO_H__
#define __JM_SYSTEM_IMPL_WIN_DISKINFO_H__
...
template<DWORD  MAX_IDE_DRIVES = 16>
class DiskInfoImpl : public boost::noncopyable
{
private:
    DiskInfoImpl()
        : m_list()
    ...
    BOOL add_if_new(USHORT *pIdSector)
    {
        BOOL bAdd = TRUE;
    ...
    std::string flip_and_code_bytes(const std::string &str)
    {
        std::string flipped;
    ...
    std::string convert_to_string(PWORD diskdata, int firstIndex, int lastIndex)
    {
        std::string str;
        ...
        for (int index = firstIndex; index <= lastIndex; index++)
        {
            ...
            c = high_byte(diskdata[index]);
            if (isalnum(c))
                str += c;

            // get low byte for 2nd character
            c = low_byte(diskdata[index]);
            if (isalnum(c))
                str += c;
        }
        return str;
    ...
    BOOL read_physical_drive_in_nt_using_smart()
    {
        BOOL done = FALSE;
        int drive = 0;
...
#define ID_CMD 0xEC            // Returns ID sector for ATA
    ...
    BOOL read_physical_drive_in_nt_with_admin_rights()
    {
        BOOL done = FALSE;
        int drive = 0;
    ...
    BOOL read_physical_drive_in_nt_with_zero_rights()
    {
        BOOL done = FALSE;
        int drive = 0;
        ...
        for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
        {
            ...
            if (hPhysicalDriveIOCTL != INVALID_HANDLE_VALUE)
            {
                ...
                if ( DeviceIoControl (hPhysicalDriveIOCTL, IOCTL_STORAGE_QUERY_PROPERTY, & query, sizeof (query), & buffer, sizeof (buffer), & cbBytesReturned, NULL) )
                {
                    ...
                    std::string serialNumber = flip_and_code_bytes(&buffer[descrip->SerialNumberOffset]);
                }
                else
                {
    ...
    BOOL read_drive_ports_in_win9x()
    {
        BOOL done = FALSE;
    ...
    BOOL read_ide_drive_as_scsi_drive_in_nt()
    {
        BOOL done = FALSE;
        int controller = 0;
...
public:
    static DiskInfoImpl& inst()
    {
        static DiskInfoImpl instance;
        return instance;
    }

    std::string model_number(size_t drive)
    {
        return convert_to_string(m_list[drive].get(), 27, 46);
    }

    std::string serial_number(size_t drive)
    {
        return convert_to_string(m_list[drive].get(), 10, 19);
    }

    std::string revision_number(size_t drive)
    {
        return convert_to_string(m_list[drive].get(), 23, 26);
    }

    std::string drive_type(size_t drive)
    {
    ...
    boost::uint64_t drive_size(size_t drive)
    {
    ...
    size_t buffer_size(size_t drive)
    {
