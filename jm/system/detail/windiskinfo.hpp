#ifndef __JM_SYSTEM_IMPL_WIN_DISKINFO_HPP__
#define __JM_SYSTEM_IMPL_WIN_DISKINFO_HPP__
...
private:
    ...
    std::string ConvertToString(PWORD diskdata, int firstIndex, int lastIndex)
    {
        ...
        for (int index = firstIndex; index <= lastIndex; index++)
        {
            ...
            c = HighByte(diskdata[index]);
            if (isalnum(c))
                str += c;

            // get low byte for 2nd character
            c = LowByte(diskdata[index]);
            if (isalnum(c))
                str += c;
        }
        return str;
...
public:
    ...
    std::string serialNumber(size_t drive)
    {
        return ConvertToString(m_list[drive].get(), 10, 19);
    }

    std::string revisionNumber(size_t drive)
    {
        return ConvertToString(m_list[drive].get(), 23, 26);
    }

    std::string driveType(size_t drive)
    {
    ...
    boost::uint64_t driveSize(size_t drive)
    {
    ...
    size_t bufferSize(size_t drive)
    {
