...
public:
    ...
    ISO14230(const boost::shared_ptr<BoxType> &box, const boost::shared_ptr<Shared> &s)
    : _box(box)
    , _shared(s)
    , _default()
    , _lLine(false)
    ...
    , _recvLine(0)
    {
        _default.reset(new DefaultFunc<BoxType, ISO14230<BoxType> >(_box, _shared, this));
    }
    ...
    boost::system::error_code fast_init(const boost::uint8_t *data, std::size_t count, boost::system::error_code &ec)
    {
        ...
        if (!_box->set_comm_ctrl(valueOpen, BoxType::Constant::SET_NULL) ||
                ...
                !_box->set_comm_link(BoxType::Constant::RS_232 | BoxType::Constant::BIT9_MARK | BoxType::Constant::SEL_SL | BoxType::Constant::UN_DB20, BoxType::Constant::SET_NULL, BoxType::Constant::SET_NULL) ||
                !_box->set_comm_baud(_opts.baud) ||
                !_box->set_comm_time(BoxType::Constant::SETBYTETIME, BoxType::to_micro_seconds(typename BoxType::MilliSeconds(5))) ||
                !_box->set_comm_time(BoxType::Constant::SETWAITTIME, BoxType::to_micro_seconds(typename BoxType::MilliSeconds(0))) ||
    ...
    boost::system::error_code addr_init(boost::uint8_t addrCode, boost::system::error_code &ec)
    {
        ...
        if (!_box->set_comm_ctrl(BoxType::Constant::PWC | BoxType::Constant::REFC |
                                 BoxType::Constant::RZFC | BoxType::Constant::CK, BoxType::Constant::SET_NULL) ||
                !_box->set_comm_link(BoxType::Constant::RS_232 | BoxType::Constant::BIT9_MARK |
                ...
                !_box->set_comm_time(BoxType::Constant::SETBYTETIME, BoxType::to_micro_seconds(typename BoxType::MilliSeconds(5))) ||
                !_box->set_comm_time(BoxType::Constant::SETWAITTIME, BoxType::to_micro_seconds(typename BoxType::MilliSeconds(12))) ||
                !_box->set_comm_time(BoxType::Constant::SETRECBBOUT, BoxType::to_micro_seconds(typename BoxType::MilliSeconds(400))) ||
                !_box->set_comm_time(BoxType::Constant::SETRECFROUT, BoxType::to_micro_seconds(typename BoxType::MilliSeconds(500))) ||
                !_box->set_comm_time(BoxType::Constant::SETLINKTIME, BoxType::to_micro_seconds(typename BoxType::MilliSeconds(500))))
        {
        ...
        if (!_box->send_out_data(&addrCode, 1) ||
                !_box->set_comm_line((_recvLine == BoxType::Constant::RK_NO) ? _sendLine : BoxType::Constant::SK_NO, _recvLine) ||
        ...
        if (!_box->run_batch(&(_shared->buffID), 1, false) ||
                (_box->read_data(temp.data(), temp.size(), BoxType::to_micro_seconds(typename BoxType::Seconds(3))) <= 0) ||
                !_box->check_result(BoxType::to_micro_seconds(typename BoxType::Seconds(5))))
        {
            _box->del_batch(_shared->buffID);
            ec = boost::asio::error::connection_refused;
            return ec;
...
private:
    ...
    boost::shared_ptr<DefaultFunc<BoxType, ISO14230<BoxType> > >_default;
    bool _lLine;
