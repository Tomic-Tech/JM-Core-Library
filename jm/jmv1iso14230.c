...
gint32 _jm_v1_iso14230_fast_init(JMKWP2000 *self, const guint8 *data, size_t count) {
    ...
    if (iso14230->l_line) {
        value_open = JM_V1_BOX_C(box, PWC) | JM_V1_BOX_C(box, RZFC) | JM_V1_BOX_C(box, CK);
    } else {
        value_open = JM_V1_BOX_C(box, PWC) | JM_V1_BOX_C(box, RZFC) | JM_V1_BOX_C(box, CK);
    }
...
gint32 _jm_v1_iso14230_set_lines(JMKWP2000 *self, gint32 com_line, gboolean l_line) {
...
static size_t _jm_v1_iso14230_read_one_frame(JMKWP2000 *self, guint8 *data, gboolean is_finish) {
    ...
    if (temp[1] == self->source_address) {
        if (temp[0] == 0x80) {
            ...
            frame_length++;
            length = box->read_bytes(box, result + JM_KWP80_HEADER_LENGTH, length + JM_KWP_CHECKSUM_LENGTH);
            frame_length += length;
        } else {
            length = temp[0] - 0x80;
            if (length <= 0) {
            ...
            memcpy(result, temp, 3);
            frame_length += 3;
            length = box->read_bytes(box, result + 3, length + JM_KWP_CHECKSUM_LENGTH);
            frame_length += length;
        }
    } else {
        if (temp[0] == 0x00) {
...
static _jm_v1_iso14230_send_one_frame(JMKWP2000 *self, const guint8 *data, size_t count, gboolean is_finish) {
    JMV1ISO14230 *iso14230 = NULL;
    JMV1Box *box = NULL;
    g_return_val_if_fail(self != NULL, 0);
    ...
    JM_V1_DEFAULT_SEND_ONE_FRAME(box->shared, box, self, data, count, is_finish, iso14230);
}
