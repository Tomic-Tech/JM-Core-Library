...
static gint32 _jm_v1_mikuni_init(JMMikuni *self)
{
    ...
    value_open = JM_V1_BOX_C(box, PWC) | JM_V1_BOX_C(box, RZFC) | 
        JM_V1_BOX_C(box, CK) | JM_V1_BOX_C(box, REFC);
...
static size_t _jm_v1_mikuni_send_one_frame(JMMikuni *self, 
    const guint8 *data, size_t count, gboolean need_recv)
{
    ...
    JM_V1_DEFAULT_SEND_ONE_FRAME(shared, box, self, data, count, 
        need_recv, mikuni);
}

size_t jm_v1_mikuni_send_one_frame(JMMikuni *self, const guint8 *data, 
    size_t count)
{
    return _jm_v1_mikuni_send_one_frame(self, data, count, FALSE);
}

size_t jm_v1_mikuni_send_frames(JMMikuni *self, const guint8 *data, 
    size_t count)
{
    return jm_v1_mikuni_send_one_frame(self, data, count);
}

static size_t _jm_v1_mikuni_read_one_frame(JMMikuni *self, guint8 *data, 
    gboolean is_finish)
{
...
gint32 jm_v1_mikuni_set_keep_link(JMMikuni *self, const guint8 *data, 
    size_t count)
{
    guint8 buff[256];
    size_t length;
