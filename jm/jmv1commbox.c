...
static JMComm* _jm_v1_commbox_configure(JMCommbox *self, 
    JMProtocolType type)
{
    JMV1Commbox *v1 = NULL;
    self->comm = NULL;
    ...
    {
        self->comm = (JMComm*)g_hash_table_lookup(v1->w80_prc_hash, 
            GINT_TO_POINTER(type));
        if (self->comm == NULL)
        {
            self->comm = jm_v1_comm_new(type, v1->w80);
            g_hash_table_insert(v1->w80_prc_hash, GINT_TO_POINTER(type), 
                self->comm);
        }
    }
    else if (v1->current_box == JM_V1_BOX_C168)
    {
        self->comm = (JMComm*)g_hash_table_lookup(v1->c168_prc_hash, 
            GINT_TO_POINTER(type));
        if (self->comm == NULL)
        {
            self->comm = jm_v1_comm_new(type, v1->c168);
            g_hash_table_insert(v1->c168_prc_hash, GINT_TO_POINTER(type), 
                self->comm);
        }
    }
...
static gint32 _jm_v1_commbox_set_connector(JMCommbox *self, 
    JMConnector type)
{
    JMV1Commbox *v1 = NULL;
    g_return_val_if_fail(self != NULL, JM_ERROR_GENERIC);
...
static gboolean _jm_v1_commbox_check_serial_port_change_config(
    JMCommbox *self)
{
    JMV1Commbox *v1 = NULL;
    g_return_val_if_fail(self != NULL, JM_ERROR_GENERIC);
...
JMCommbox* jm_v1_commbox_new(void)
{
    ...
    obj->serial_port_flow_control = 
        _jm_v1_commbox_serial_port_flow_control;
    ...
    v1->w80_prc_hash = g_hash_table_new_full(g_direct_hash, g_direct_equal, 
        NULL, _jm_comm_destroy);
    v1->c168_prc_hash = g_hash_table_new_full(g_direct_hash, g_direct_equal, 
        NULL, _jm_comm_destroy);
