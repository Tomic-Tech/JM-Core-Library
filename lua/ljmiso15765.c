...
static int _lua_jm_iso15765_set_options(lua_State *L) {
    ...
    JMCanbus *canbus = (JMCanbus*)jm_comm_get_protocol(comm);
    ...
    if (comm->prc_type != JM_PRC_ISO15765) {
        lua_pushboolean(L, 0);
        return 1;
    }
    ...
    if (lua_jm_canbus_baud[baud_i].name == NULL ||
        ...
        if (jm_canbus_set_options(canbus, target_id,
...
static int _lua_jm_iso15765_set_lines(lua_State *L) {
    ...
    if (jm_canbus_set_lines(canbus, high, low) != JM_ERROR_SUCCESS) {
        //        lua_pushstring(L, "JMLib commbox setting iso15765 lines fail!");
        //        lua_error(L);
        lua_pushboolean(L, 0);
    } else {
        lua_pushboolean(L, 1);
...
static int _lua_jm_iso15765_set_filter(lua_State *L) {
    ...
    while (TRUE) {
        ...
        lua_gettable(L, -2);
        if (!lua_isnil(L, -1) && lua_isnumber(L, -1) && i != 16) { /* Check if the last table key or none number */
    ...
    if (i == 0)
        lua_pushboolean(L, 0);
    if (jm_canbus_set_filter(canbus, id, i) != JM_ERROR_SUCCESS) {
        lua_pushboolean(L, 0);
    } else {
        lua_pushboolean(L, 1);
    }
